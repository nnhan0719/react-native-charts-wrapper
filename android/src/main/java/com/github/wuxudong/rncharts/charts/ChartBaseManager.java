...
public abstract class ChartBaseManager<T extends Chart, U extends Entry> extends SimpleViewManager<T> {
    ...
    private void setMarkerParams(RNRectangleMarkerView marker, ReadableMap propMap) {
        ...
        if (BridgeUtils.validate(propMap, ReadableType.Number, "markerColor")) {
            marker.setTooltipBackground(propMap.getInt("markerColor"));
        }
        if (BridgeUtils.validate(propMap, ReadableType.Number, "customColor")) {
            marker.setTitleColor(propMap.getInt("customColor"));
        }
        if (BridgeUtils.validate(propMap, ReadableType.Boolean, "isCredit")) {
            marker.setDotColor(propMap.getBoolean("isCredit"));
        }
